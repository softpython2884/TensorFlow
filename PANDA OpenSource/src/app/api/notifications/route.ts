...
export async function GET(request: NextRequest) {
  ...
  try {
    const podResponse = await fetch(`${POD_API_URL}/api/pod/users/me/notifications`, {
      method: 'GET',
      headers: {
    ...
    if (!podResponse.ok) {
      return NextResponse.json({ error: podData.error || 'Failed to fetch notifications from Pod' }, { status: podResponse.status });
    }
    return NextResponse.json(podData);

  } catch (error) {
    console.error('BFF Fetch Notifications error:', error);
