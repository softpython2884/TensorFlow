...
import { SignJWT } from 'jose'; // For setting cookie, actual token generation is in Pod

const POD_API_URL = ENV.NEXT_PUBLIC_APP_URL; // Assuming Pod is on the same domain or configured
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { token, user } = podData; // Pod returns the JWT and user details
